...
const highOrderRequestOnFullfilledInterceptor = (dispatch, BASE_URL) => {
  const requestOnFullfilledInterceptor = config => {
    const requestUrlConfigObject = (0, _mapRequestEndpointToConfigObject.default)(config, _apiRequestsConfigs.default);
...
const highOrderResponseOnFullfilledInterceptor = (dispatch, BASE_URL) => {
  const responseOnFullfilledInterceptor = response => {
    const requestUrlConfigObject = (0, _mapRequestEndpointToConfigObject.default)(response.config, _apiRequestsConfigs.default);
...
const highOrderResponseOnRejectedInterceptor = (dispatch, BASE_URL) => {
  const responseOnRejectedInterceptor = error => {
    const requestUrlConfigObject = (0, _mapRequestEndpointToConfigObject.default)(error.config, _apiRequestsConfigs.default);
