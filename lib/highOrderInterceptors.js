...
const highOrderResponseOnFullfilledInterceptor = (dispatch, BASE_URL) => {
  const responseOnFullfilledInterceptor = response => {
    const requestUrlConfigObject = (0, _mapRequestEndpointToConfigObject.default)(response.config.url.includes(BASE_URL) ? response.config.url.split(BASE_URL)[1] : response.config.url, _apiRequestsConfigs.default);
...
const highOrderResponseOnRejectedInterceptor = (dispatch, BASE_URL) => {
  const responseOnRejectedInterceptor = error => {
    const requestUrlConfigObject = (0, _mapRequestEndpointToConfigObject.default)(error.config.url.includes(BASE_URL) ? error.config.url.split(BASE_URL)[1] : error.config.url, _apiRequestsConfigs.default);
